...
class MergedDataset(Dataset):
    def __init__(self, dir_1, dir_1_sep, dir_2, dir_2_sep, size, transform=None, target_transform=None, phase='train', wandb_ex=None):
        ...
        # second dataet must have fewer insctances
    ...
    def __getitem__(self, idx):
        ...
        image = np.load(os.path.join(
            data['img_dir'], data['file_name']))['arr_0']
        if label:
            mask = np.load(os.path.join(
                data['mask_dir'], data['file_name']))['arr_0']
        else:
            mask = np.load(os.path.join(data['mask_dir'], data['file_name']))[
                'arr_0']*255
        if image.ndim == 2:
            image = image[:, :, None]
