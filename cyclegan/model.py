...
class ResnetBlock(nn.Module):
    def __init__(self, dim, padding_type='reflect', norm_type=None, use_dropout=True, dropout_rate=0.25):
        ...
        self.conv_block = self.build_conv_block(
            dim, padding_type, norm_type, use_dropout, dropout_rate)
    ...
    def build_conv_block(self, dim, padding_type, norm_type, use_dropout, dropout_rate):
        ...
        conv_block += [GeneralConv2d(dim, dim,
                                     kernel_size=3, stride=1, padding=p)]
...
class ResnetBlockDS(ResnetBlock):
    def __init__(self, dim_in, dim_out, padding_type='reflect', norm_type=None, use_dropout=True, dropout_rate=0.25):
        super(ResnetBlockDS, self).__init__(
            dim_out, padding_type, norm_type, use_dropout, dropout_rate)
        self.expand_dim = nn.Conv2d(
            dim_in, dim_out, kernel_size=1, stride=1, padding=0) if dim_in != dim_out else None
...
class DRNBlock(nn.Module):
    def __init__(self, dim, norm_layer=None, dropout_rate=0.25):
        ...
        self.dilated_conv1 = DilateConv2d(dim, dim, kernel_size=3, stride=1, dilation=dilation,
                                          bias=False, norm_layer=norm_layer, activation=nn.ReLU(True), dropout_rate=dropout_rate)

        self.pad2 = nn.ReflectionPad2d(padding)
        self.dilated_conv2 = DilateConv2d(dim, dim, kernel_size=3, stride=1, dilation=dilation,
                                          bias=False, norm_layer=norm_layer, activation=None, dropout_rate=dropout_rate)
...
class DRNBlockDS(nn.Module):
    def __init__(self, dim_in, dim_out, norm_layer=None, dropout_rate=0.25):
        ...
        self.pad1 = nn.ReflectionPad2d(padding)
        self.dilated_conv1 = DilateConv2d(dim_in, dim_out, kernel_size=3, stride=1, dilation=dilation,
                                          bias=False, norm_layer=norm_layer, activation=nn.ReLU(True), dropout_rate=dropout_rate)

        self.pad2 = nn.ReflectionPad2d(padding)
        self.dilated_conv2 = DilateConv2d(dim_out, dim_out, kernel_size=3, stride=1, dilation=dilation,
                                          bias=False, norm_layer=norm_layer, activation=None, dropout_rate=dropout_rate)
...
class DiscriminatorAux(nn.Module):
    def __init__(self, input_nc=1, ndf=64, norm_layer=nn.BatchNorm2d):
        ...
        self.conv1 = GeneralConv2d(input_nc, ndf, kernel_size=4, stride=2, padding=0,
                                   norm_layer=norm_layer, use_bias=False, activation=nn.LeakyReLU(0.2, True))

        self.conv2 = GeneralConv2d(ndf, ndf * 2, kernel_size=4, stride=2, padding=0,
                                   norm_layer=norm_layer, use_bias=False, activation=nn.LeakyReLU(0.2, True))
        self.conv3 = GeneralConv2d(ndf * 2, ndf * 4, kernel_size=4, stride=2, padding=0,
                                   norm_layer=norm_layer, use_bias=False, activation=nn.LeakyReLU(0.2, True))
        self.conv4 = GeneralConv2d(ndf * 4, ndf * 8, kernel_size=4, stride=1, padding=0,
                                   norm_layer=norm_layer, use_bias=False, activation=nn.LeakyReLU(0.2, True))
        self.conv5 = GeneralConv2d(
            ndf * 8, 2, kernel_size=4, stride=1, padding=0, use_bias=False, activation=None)
...
class ResnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc=1, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=9, padding_type='reflect', skip=False):
        ...
        for i in range(n_blocks):
            model += [ResnetBlockIns(ngf * mult, padding_type=padding_type,
                                     norm_type='instance', use_dropout=use_dropout, dropout_rate=0.25)]

        # upsample convolutions
        for i in range(n_downsampling):
            mult = 2**(n_downsampling - i)
            model += [GeneralDeconv2d(ngf * mult, int(ngf * mult / 2),
                                      kernel_size=3, stride=2, padding=1)]

        model += [nn.ReflectionPad2d(3),
                  GeneralConv2dGA(ngf, output_nc, kernel_size=7,
                                  padding=0, do_relu=False),
                  ]
        self.model = nn.Sequential(*model)
...
class Decoder(nn.Module):
    def __init__(self, input_nc, ngf=64, norm_layer=nn.InstanceNorm2d, use_bias=False, skip=False):
        ...
        self.conv1 = GeneralConv2d(input_nc, ngf * 4, ks, stride=1, padding=ks //
                                   2, norm_layer=norm_layer, use_bias=use_bias, activation=nn.ReLU(True))
        self.res1 = ResnetBlock(
            ngf * 4, padding_type=padding_type, norm_layer=norm_layer, use_bias=use_bias)
        ...
        self.res4 = ResnetBlock(
            ngf * 4, padding_type=padding_type, norm_layer=norm_layer, use_bias=use_bias)
        self.deconv1 = GeneralDeconv2d(ngf * 4, ngf * 2, ks, stride=2, padding=ks//2,
                                       output_padding=1, norm_layer=norm_layer, use_bias=use_bias, activation=nn.ReLU(True))
        self.deconv2 = GeneralDeconv2d(ngf * 2, ngf * 2, ks, stride=2, padding=ks//2,
                                       output_padding=1, norm_layer=norm_layer, use_bias=use_bias, activation=nn.ReLU(True))
        self.deconv3 = GeneralDeconv2d(ngf * 2, ngf, ks, stride=2, padding=ks//2, output_padding=1,
                                       norm_layer=norm_layer, use_bias=use_bias, activation=nn.ReLU(True))
        self.conv2 = GeneralConv2d(
            ngf, 1, f, stride=1, padding=f//2, norm_layer=None, use_bias=use_bias, activation=None)
...
class Segmenter(nn.Module):
    def __init__(self, input_nc, keep_rate=0.75,drop_out_rate=0):
        super(Segmenter, self).__init__()

        self.conv = GeneralConv2d(input_nc, 5, kernel_size=1, stride=1,
                                  padding=0, norm_layer=None, use_bias=False, activation=None)
...
class Encoder(nn.Module):
    def __init__(self, input_dim, name='encoder', is_training=True, keep_rate=0.75,drop_out_rate=0):
        ...
        self.r11 = ResnetBlockDS(
            self.fb*16, self.fb*32, padding_type=self.padding)
        self.r12 = ResnetBlock(self.fb*32, padding_type=self.padding)
        self.d1 = DRNBlock(self.fb*32)
        self.d2 = DRNBlock(self.fb*32)
        self.c2 = GeneralConv2d(self.fb*32, self.fb*32,
                                self.k1, 1, stddev=0.01)
        self.c3 = GeneralConv2d(self.fb*32, self.fb*32,
                                self.k1, 1, stddev=0.01)
...
class SIFAModule(nn.Module):
    def __init__(self, n_channels, n_classes, skip, bilinear=False):
        ...
        self.encoder_B = Encoder(skip=skip, drop_out_rate=self.drop_out_rate)
        self.decoder_B = Decoder(skip=skip)
        self.segmenter_B = Segmenter(drop_out_rate=self.drop_out_rate)
        self.segmenter_B_ll = Segmenter(drop_out_rate=self.drop_out_rate)
    ...
    def forward(self, inputs):
        ...
        prob_real_a_is_real, prob_real_a_aux = self.discriminator_aux_A(
            images_a)
        prob_real_b_is_real = self.discriminator_B(images_b)
        fake_images_b = self.generator_A(images_a, images_a,)
        latent_b, latent_b_ll = self.encoder_B(images_b, )
        ...
        prob_fake_a_is_real, prob_fake_a_aux_is_real = self.discriminator_aux_A(
            fake_images_a)
        prob_fake_b_is_real = self.discriminator_B(fake_images_b)
        latent_fake_b, latent_fake_b_ll = self.encoder_B(fake_images_b)
        cycle_images_b = self.generator_A(fake_images_a, fake_images_a)
        cycle_images_a = self.decoder_B(latent_fake_b, fake_images_b)
        pred_mask_fake_b = self.segmenter_B(latent_fake_b,)
        pred_mask_fake_b_ll = self.segmenter_B_ll(latent_fake_b_ll,)
        prob_fake_pool_a_is_real, prob_fake_pool_a_aux_is_real = self.discriminator_aux_A(
            fake_pool_a)
        prob_fake_pool_b_is_real = self.discriminator_B(fake_pool_b)
        prob_cycle_a_is_real, prob_cycle_a_aux_is_real = self.discriminator_aux_A(
            cycle_images_a)
        ...
        prob_pred_mask_fake_b_ll_is_real = self.discriminator_P_ll(
            pred_mask_fake_b_ll)
